import { OrderStatus, ProviderProfile } from "@prisma/client";
...
const updateProviderProfileInDB = async (userId: string, data: Partial<ProviderProfile>) => {
...
const getProviderDashboardStatsFromDB = async (userId: string) => {
    ...
    const [revenueData, activeOrdersCount, customersData, reviewsData] = await Promise.all([
        // Total Revenue
